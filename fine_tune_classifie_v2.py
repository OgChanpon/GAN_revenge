# fine_tune_classifier_v2.py (修正済み: AttributeError解決版)
...
# --- 学習ループ ---
print("\n--- Fine-tuning Start (with Early Stopping) ---")
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        best_model_wts = copy.deepcopy(discriminator.state_dict())
        counter = 0 
        print(f"  -> Best Validation Accuracy Updated: {best_val_acc:.2f}% (Model Saved in Memory)")
    else:
        counter += 1
        print(f"  -> No improvement. Patience: {counter}/{patience}")
        
    if counter >= patience:
        print("Early Stopping: これ以上精度が上がらないため終了します。")
        break

# --- ベストモデルの保存 ---
print(f"\n最も検証精度の良かったモデルを保存します... (Val Acc: {best_val_acc:.2f}%)")
